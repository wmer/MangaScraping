                            ...
                            foreach (var upd in updates) {
                                ...
                                if (_libraryContext.Hq.Find().Where(x => x.Link == link).Execute().FirstOrDefault() is Hq hq) {
                                    ...
                                    foreach (var chap in upd.Chapters) {
                                        ...
                                        if (_libraryContext.Chapter.Find().Where(x => x.Link == chap.Link).Execute().FirstOrDefault() is Chapter chapDb) {
                                            chap.Id = chapDb.Id;
                                        } else {
                                            _libraryContext.Chapter.Save(chap);
                                            var chapId = _libraryContext.Chapter.Find().Where(x => x.Link == chap.Link).Execute().FirstOrDefault().Id;
                                            chap.Id = Convert.ToInt32(chapId);
                                        }
                                    }
                                } else {
